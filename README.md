...
- `wrong_seq`, `wrong_ack`, `wrong_timestamp`, `wrong_checksum`, and `wrong_md5` send a fake decoy ClientHello during the TCP handshake path. DPI may inspect it, but the real upstream server should discard it.
